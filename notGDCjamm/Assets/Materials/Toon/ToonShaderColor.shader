Shader "Custom/ToonShaderColor"
{
	Properties{
		...
		_Outline("Outline width", Range(0, 0.1)) = .0005
		...
		SubShader{
			...
			v2f vert(appdata v)
			{
				...
				//float2 offset = TransformViewToProjection(_WorldSpaceCameraPos.xy); //norm.xy
				o.pos = UnityObjectToClipPos(v.vertex+ normalize(v.normal) * _Outline);
		...
		Pass
		{
				...
				half4 frag(v2f i) :COLOR
				{
					fixed4 col = _ShadowColor+ (i.color *Toon(i.worldNormal, _WorldSpaceLightPos0.xyz)*_Strength*_Color + _Brightness) ;
					
					
					return col ;
					//return col; //works with DirLight
...
/*
			// Pass to render object as a shadow caster //casts shadow on Forward/Defered
				Pass
				{
					Name "ShadowCaster"
					Tags { "LightMode" = "ShadowCaster" }
